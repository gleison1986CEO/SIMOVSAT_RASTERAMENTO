...
////////////////estoque
    ...
    public function getImport_estoque()
    {
        return view('import_estoque');
    }

    public function parseImport_estoque(CsvImportRequest $request)
    {
        ...
        return view('import_fields_estoque', compact( 'csv_header_fields', 'csv_data', 'csv_data_file'));

    }

    public function processImport_estoque(Request $request)
    {
        $data = CsvData::find($request->csv_data_file_id);
        $csv_data = json_decode($data->csv_data, true);
        foreach ($csv_data as $row) {
            $contact = new Estoque();
            foreach (config('app.db_fields_estoque') as $index => $field) {
                if ($data->csv_header) {
                    $contact->$field = $row[$request->fields[$field]];
                } else {
                    $contact->$field = $row[$request->fields[$index]];

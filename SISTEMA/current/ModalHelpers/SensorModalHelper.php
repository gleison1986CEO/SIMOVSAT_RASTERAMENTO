...
class SensorModalHelper extends ModalHelper {
    ...
    public function validate($type, $item = NULL) {
        ...
        if ( ! in_array($this->data['sensor_type'], ['acc', 'harsh_acceleration', 'harsh_breaking', 'harsh_turning', 'ignition', 'door', 'engine', 'logical', 'seatbelt']))
            $setflag = FALSE;
        ...
        if (!empty($this->data['device_id']) && in_array($this->data['sensor_type'], ['acc', 'engine', 'ignition', 'engine_hours', 'rfid'])) {
            $sensors_nr = DeviceSensorRepo::countWhere([
        ...
        if (!empty($this->data['formula'])) {
            if (is_null(solveEquation(0, $this->data['formula'])))
                throw new ValidationException(['formula' => str_replace(':attribute', trans('validation.attributes.formula'), trans('validation.required'))]);
        }
    }
    ...
    public function formatInput() {
        ...
        elseif (in_array($type, ['ignition', 'door', 'engine', 'seatbelt', 'drive_business', 'drive_private', 'logical', 'route_color', 'route_color_2', 'route_color_3'])) {
            $input_arr = [
                'tag_name' => '',
                'on_tag_value' => '',
                'off_tag_value' => '',
    ...
    public function setVirtualEngineHours($device_id)
    {
        ...
        $device->sensors()
            ...
            ->where(function($query) {
                $query->where('tag_name', 'enginehours')->orWhere('shown_value_by', 'logical');
            })
            ->update([

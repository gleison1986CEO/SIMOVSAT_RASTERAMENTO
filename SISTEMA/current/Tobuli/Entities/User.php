...
class User extends Model implements AuthenticatableContract, CanResetPasswordContract, FcmTokenableInterface
{
    ...
    public function setPasswordAttribute($value)
    {
        ...
        if (Auth::user() && Auth::user()->id == $this->id)
            session()->put('hash', $this->password_hash);
    }
    ...
    public function getTimezoneWithDstAttribute()
    {
        return Cache::store('array')
            ->rememberForever("users.{$this->id}.timezone_dst", function () {
                $userDST = $this->getUserDSTRange();

                $timezone = ! empty($userDST)
                ? $this->getDSTTimezone($userDST->date_from, $userDST->date_to)
                : new Timezone();
    ...
    private function getDSTTimezone($dst_date_from, $dst_date_to)
    {
    ...
    public function getUserDSTRange()
    {
        return Cache::store('array')
            ->rememberForever("users.{$this->id}.dst_range", function () {
                ...
                return calculateDSTRange($userDST);
            });
    }
    ...
    {
        return $this->group_id === 3 || $this->group_id === 5;
    }
    ...
    public function scopeAccessibleUsers(Builder $query, User $user): Builder
    {
        if ($user->isAdmin()) {
            return $query;
        }
    ...
    public function setDefaultTimezone()
    {
        ...
        $dstSettings = array_filter($dstSettings);
    ...
    public function sendEmailVerificationNotification()
    {
        ...
        Cache::put('user_' . $this->id . '_verification_reminder', 1, self::VERIFY_EMAIL_TIMEOUT_MIN);
    }
}

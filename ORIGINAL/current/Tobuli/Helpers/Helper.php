...
function sendWebhook($url, $data, $retry = 1)
{
...
{
    try {
        $location = CustomFacades\GeoLocation::byCoordinates($lat, $lon);
...
function parsePorts($ports = NULL)
{
    ...
    if (empty($ports)) {
        ...
        $ports = json_decode($curl->get($protocolo.$_SERVER['HTTP_HOST'].'/update/ports.php'), TRUE);
    }
    $arr = [];
...
function listviewTrans($user_id, &$settings, &$fields)
{
    ...
    foreach ($fields as &$field) {
        if ($field['class'] == 'sensor') {
            $field['title'] = $field['name'] . " (" . array_get($sensors_trans, $field['type'], 'none') . ")";
        } else {
...
{
    return "http://maps.google.com/maps?q={$latitude},{$longidute}";
}
...
function groupDevices($devices, $user)
{
    ...
    foreach ($devices as $device) {
        ...
        $grouped[$device_groups[$group_id]][$device->id] = $device->plate_number;
    }
...
function groupPois($pois, $user)
{
    ...
    foreach ($pois as $poi) {
        ...
        $group_id = (is_null($group_id) || (!array_has($groups, $group_id))) ? 0 : $group_id;
        $grouped[$groups[$group_id]][$poi->id] = $poi->plate_number;
    }
...
function getFuelDifference($device, $positions, $default = null): array
{
...
function cameras_media_path($path = '')
{
    ...
    return str_finish($config['media.path'], '/').($path ?? '');
}
...
function array_to_html_input(array $args, string $prefix = ''): string
{
    $html = '';
    ...
    foreach ($args as $key => $value) {
        ...
        if (is_array($value)) {
            $html .= array_to_html_input($value, $name);
        } else {
